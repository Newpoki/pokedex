import { rest } from "msw";
import { Item } from "../../../item/typings";

const mockedDefaultItemData: Item = {
  attributes: [
    { name: "countable", url: "https://pokeapi.co/api/v2/item-attribute/1/" },
    { name: "consumable", url: "https://pokeapi.co/api/v2/item-attribute/2/" },
    { name: "usable-overworld", url: "https://pokeapi.co/api/v2/item-attribute/3/" },
    { name: "usable-in-battle", url: "https://pokeapi.co/api/v2/item-attribute/4/" },
    { name: "holdable", url: "https://pokeapi.co/api/v2/item-attribute/5/" },
  ],
  baby_trigger_for: null,
  category: { name: "vitamins", url: "https://pokeapi.co/api/v2/item-category/26/" },
  cost: 10000,
  effect_entries: [
    {
      effect:
        "Used on a party Pokémon\n:   Increases a selected move's max PP by 20% its original max PP, to a maximum of 1.6×.  Increases happiness by 5/3/2.",
      language: { name: "en", url: "https://pokeapi.co/api/v2/language/9/" },
      short_effect: "Raises a move's max PP by 20%.",
    },
  ],
  flavor_text_entries: [
    {
      language: { name: "en", url: "https://pokeapi.co/api/v2/language/9/" },
      text: "Raises the maximum\nPP of a selected\nmove.",
      version_group: { name: "ruby-sapphire", url: "https://pokeapi.co/api/v2/version-group/5/" },
    },
    {
      language: { name: "en", url: "https://pokeapi.co/api/v2/language/9/" },
      text: "Raises the maximum\nPP of a selected\nmove.",
      version_group: { name: "emerald", url: "https://pokeapi.co/api/v2/version-group/6/" },
    },
    {
      language: { name: "en", url: "https://pokeapi.co/api/v2/language/9/" },
      text: "Slightly raises the maximum PP of\na selected move for one POKéMON.",
      version_group: {
        name: "firered-leafgreen",
        url: "https://pokeapi.co/api/v2/version-group/7/",
      },
    },
    {
      language: { name: "en", url: "https://pokeapi.co/api/v2/language/9/" },
      text: "It slightly raises the maximum PP of\na selected move that has been learned\nby the target Pokémon.",
      version_group: { name: "diamond-pearl", url: "https://pokeapi.co/api/v2/version-group/8/" },
    },
    {
      language: { name: "en", url: "https://pokeapi.co/api/v2/language/9/" },
      text: "It slightly raises the maximum PP of\na selected move that has been learned\nby the target Pokémon.",
      version_group: { name: "platinum", url: "https://pokeapi.co/api/v2/version-group/9/" },
    },
    {
      language: { name: "en", url: "https://pokeapi.co/api/v2/language/9/" },
      text: "It slightly raises the maximum PP of\na selected move that has been learned\nby the target Pokémon.",
      version_group: {
        name: "heartgold-soulsilver",
        url: "https://pokeapi.co/api/v2/version-group/10/",
      },
    },
    {
      language: { name: "fr", url: "https://pokeapi.co/api/v2/language/5/" },
      text: "Monte un peu les PP max d’une\ncapacité sélectionnée.",
      version_group: { name: "black-white", url: "https://pokeapi.co/api/v2/version-group/11/" },
    },
    {
      language: { name: "en", url: "https://pokeapi.co/api/v2/language/9/" },
      text: "It slightly raises the maximum PP of\na selected move that has been learned\nby the target Pokémon.",
      version_group: { name: "black-white", url: "https://pokeapi.co/api/v2/version-group/11/" },
    },
    {
      language: { name: "en", url: "https://pokeapi.co/api/v2/language/9/" },
      text: "It slightly raises the maximum PP of\na selected move that has been learned\nby the target Pokémon.",
      version_group: {
        name: "black-2-white-2",
        url: "https://pokeapi.co/api/v2/version-group/14/",
      },
    },
    {
      language: { name: "ja-Hrkt", url: "https://pokeapi.co/api/v2/language/1/" },
      text: "ポケモンが　おぼえている\nわざのうち　１つの　ＰＰの\nさいだいちを　すこし　あげる。",
      version_group: { name: "x-y", url: "https://pokeapi.co/api/v2/version-group/15/" },
    },
    {
      language: { name: "ko", url: "https://pokeapi.co/api/v2/language/3/" },
      text: "포켓몬이 기억하고 있는\n기술 중 1개의 PP\n최대치를 조금 올린다.",
      version_group: { name: "x-y", url: "https://pokeapi.co/api/v2/version-group/15/" },
    },
    {
      language: { name: "fr", url: "https://pokeapi.co/api/v2/language/5/" },
      text: "Augmente un peu les PP max d’une capacité\nsélectionnée.",
      version_group: { name: "x-y", url: "https://pokeapi.co/api/v2/version-group/15/" },
    },
    {
      language: { name: "de", url: "https://pokeapi.co/api/v2/language/6/" },
      text: "Erhöht die maximale Anzahl der AP einer Attacke\neines Pokémon.",
      version_group: { name: "x-y", url: "https://pokeapi.co/api/v2/version-group/15/" },
    },
    {
      language: { name: "es", url: "https://pokeapi.co/api/v2/language/7/" },
      text: "Aumenta ligeramente los PP máximos del\nmovimiento elegido de un Pokémon.",
      version_group: { name: "x-y", url: "https://pokeapi.co/api/v2/version-group/15/" },
    },
    {
      language: { name: "it", url: "https://pokeapi.co/api/v2/language/8/" },
      text: "Aumenta di poco i PP massimi di una mossa\na scelta tra quelle conosciute dal Pokémon.",
      version_group: { name: "x-y", url: "https://pokeapi.co/api/v2/version-group/15/" },
    },
    {
      language: { name: "en", url: "https://pokeapi.co/api/v2/language/9/" },
      text: "A medicine that can slightly raise the\nmaximum PP of a single move that has\nbeen learned by the target Pokémon.",
      version_group: { name: "x-y", url: "https://pokeapi.co/api/v2/version-group/15/" },
    },
    {
      language: { name: "ja", url: "https://pokeapi.co/api/v2/language/11/" },
      text: "ポケモンが　覚えている\n技のうち　１つの　ＰＰの\n最大値を　少し　あげる。",
      version_group: { name: "x-y", url: "https://pokeapi.co/api/v2/version-group/15/" },
    },
    {
      language: { name: "ja-Hrkt", url: "https://pokeapi.co/api/v2/language/1/" },
      text: "ポケモンが　おぼえている\nわざのうち　１つの　ＰＰの\nさいだいちを　すこし　あげる。",
      version_group: { name: "sun-moon", url: "https://pokeapi.co/api/v2/version-group/17/" },
    },
    {
      language: { name: "ko", url: "https://pokeapi.co/api/v2/language/3/" },
      text: "포켓몬이 기억하고 있는\n기술 중 1개의 PP\n최대치를 조금 올린다.",
      version_group: { name: "sun-moon", url: "https://pokeapi.co/api/v2/version-group/17/" },
    },
    {
      language: { name: "zh-Hant", url: "https://pokeapi.co/api/v2/language/4/" },
      text: "能讓寶可夢已學會的\n其中１個招式的\nＰＰ最大值少量提高。",
      version_group: { name: "sun-moon", url: "https://pokeapi.co/api/v2/version-group/17/" },
    },
    {
      language: { name: "fr", url: "https://pokeapi.co/api/v2/language/5/" },
      text: "Augmente un peu les PP max d’une capacité\nsélectionnée.",
      version_group: { name: "sun-moon", url: "https://pokeapi.co/api/v2/version-group/17/" },
    },
    {
      language: { name: "de", url: "https://pokeapi.co/api/v2/language/6/" },
      text: "Erhöht die maximale Anzahl der AP einer Attacke\neines Pokémon.",
      version_group: { name: "sun-moon", url: "https://pokeapi.co/api/v2/version-group/17/" },
    },
    {
      language: { name: "es", url: "https://pokeapi.co/api/v2/language/7/" },
      text: "Aumenta ligeramente los PP máximos del\nmovimiento elegido de un Pokémon.",
      version_group: { name: "sun-moon", url: "https://pokeapi.co/api/v2/version-group/17/" },
    },
    {
      language: { name: "it", url: "https://pokeapi.co/api/v2/language/8/" },
      text: "Aumenta di poco i PP massimi di una mossa\na scelta tra quelle conosciute dal Pokémon.",
      version_group: { name: "sun-moon", url: "https://pokeapi.co/api/v2/version-group/17/" },
    },
    {
      language: { name: "en", url: "https://pokeapi.co/api/v2/language/9/" },
      text: "A medicine that can slightly raise the maximum PP\nof a single move that has been learned by the\ntarget Pokémon.",
      version_group: { name: "sun-moon", url: "https://pokeapi.co/api/v2/version-group/17/" },
    },
    {
      language: { name: "ja", url: "https://pokeapi.co/api/v2/language/11/" },
      text: "ポケモンが　覚えている\n技のうち　１つの　ＰＰの\n最大値を　少し　あげる。",
      version_group: { name: "sun-moon", url: "https://pokeapi.co/api/v2/version-group/17/" },
    },
    {
      language: { name: "zh-Hans", url: "https://pokeapi.co/api/v2/language/12/" },
      text: "能让宝可梦学会的\n其中１个招式的\nＰＰ最大值少量提高。",
      version_group: { name: "sun-moon", url: "https://pokeapi.co/api/v2/version-group/17/" },
    },
    {
      language: { name: "ja-Hrkt", url: "https://pokeapi.co/api/v2/language/1/" },
      text: "ポケモンが　おぼえている\nわざのうち　１つの　ＰＰの\nさいだいちを　すこし　あげる。",
      version_group: {
        name: "ultra-sun-ultra-moon",
        url: "https://pokeapi.co/api/v2/version-group/18/",
      },
    },
    {
      language: { name: "ko", url: "https://pokeapi.co/api/v2/language/3/" },
      text: "포켓몬이 기억하고 있는\n기술 중 1개의 PP\n최대치를 조금 올린다.",
      version_group: {
        name: "ultra-sun-ultra-moon",
        url: "https://pokeapi.co/api/v2/version-group/18/",
      },
    },
    {
      language: { name: "zh-Hant", url: "https://pokeapi.co/api/v2/language/4/" },
      text: "能讓寶可夢已學會的\n其中１個招式的\nＰＰ最大值少量提高。",
      version_group: {
        name: "ultra-sun-ultra-moon",
        url: "https://pokeapi.co/api/v2/version-group/18/",
      },
    },
    {
      language: { name: "fr", url: "https://pokeapi.co/api/v2/language/5/" },
      text: "Augmente un peu les PP max d’une capacité\nsélectionnée.",
      version_group: {
        name: "ultra-sun-ultra-moon",
        url: "https://pokeapi.co/api/v2/version-group/18/",
      },
    },
    {
      language: { name: "de", url: "https://pokeapi.co/api/v2/language/6/" },
      text: "Erhöht die maximale Anzahl der AP einer Attacke\neines Pokémon.",
      version_group: {
        name: "ultra-sun-ultra-moon",
        url: "https://pokeapi.co/api/v2/version-group/18/",
      },
    },
    {
      language: { name: "es", url: "https://pokeapi.co/api/v2/language/7/" },
      text: "Aumenta ligeramente los PP máximos del\nmovimiento elegido de un Pokémon.",
      version_group: {
        name: "ultra-sun-ultra-moon",
        url: "https://pokeapi.co/api/v2/version-group/18/",
      },
    },
    {
      language: { name: "it", url: "https://pokeapi.co/api/v2/language/8/" },
      text: "Aumenta di poco i PP massimi di una mossa\na scelta tra quelle conosciute dal Pokémon.",
      version_group: {
        name: "ultra-sun-ultra-moon",
        url: "https://pokeapi.co/api/v2/version-group/18/",
      },
    },
    {
      language: { name: "en", url: "https://pokeapi.co/api/v2/language/9/" },
      text: "A medicine that can slightly raise the maximum PP\nof a single move that has been learned by the\ntarget Pokémon.",
      version_group: {
        name: "ultra-sun-ultra-moon",
        url: "https://pokeapi.co/api/v2/version-group/18/",
      },
    },
    {
      language: { name: "ja", url: "https://pokeapi.co/api/v2/language/11/" },
      text: "ポケモンが　覚えている\n技のうち　１つの　ＰＰの\n最大値を　少し　あげる。",
      version_group: {
        name: "ultra-sun-ultra-moon",
        url: "https://pokeapi.co/api/v2/version-group/18/",
      },
    },
    {
      language: { name: "zh-Hans", url: "https://pokeapi.co/api/v2/language/12/" },
      text: "能让宝可梦学会的\n其中１个招式的\nＰＰ最大值少量提高。",
      version_group: {
        name: "ultra-sun-ultra-moon",
        url: "https://pokeapi.co/api/v2/version-group/18/",
      },
    },
    {
      language: { name: "ja-Hrkt", url: "https://pokeapi.co/api/v2/language/1/" },
      text: "ポケモンが　おぼえている\nわざのうち　１つの　ＰＰの\nさいだいちを　すこし　あげる。",
      version_group: {
        name: "lets-go-pikachu-lets-go-eevee",
        url: "https://pokeapi.co/api/v2/version-group/19/",
      },
    },
    {
      language: { name: "ko", url: "https://pokeapi.co/api/v2/language/3/" },
      text: "포켓몬이 기억하고 있는\n기술 중 1개의 PP\n최대치를 조금 올린다.",
      version_group: {
        name: "lets-go-pikachu-lets-go-eevee",
        url: "https://pokeapi.co/api/v2/version-group/19/",
      },
    },
    {
      language: { name: "zh-Hant", url: "https://pokeapi.co/api/v2/language/4/" },
      text: "能讓寶可夢已學會的\n其中１個招式的\nPP最大值少量提高。",
      version_group: {
        name: "lets-go-pikachu-lets-go-eevee",
        url: "https://pokeapi.co/api/v2/version-group/19/",
      },
    },
    {
      language: { name: "fr", url: "https://pokeapi.co/api/v2/language/5/" },
      text: "Augmente un peu les PP max d’une capacité\nsélectionnée.",
      version_group: {
        name: "lets-go-pikachu-lets-go-eevee",
        url: "https://pokeapi.co/api/v2/version-group/19/",
      },
    },
    {
      language: { name: "de", url: "https://pokeapi.co/api/v2/language/6/" },
      text: "Erhöht die maximale Anzahl der AP einer Attacke\neines Pokémon.",
      version_group: {
        name: "lets-go-pikachu-lets-go-eevee",
        url: "https://pokeapi.co/api/v2/version-group/19/",
      },
    },
    {
      language: { name: "es", url: "https://pokeapi.co/api/v2/language/7/" },
      text: "Aumenta ligeramente los PP máximos del\nmovimiento elegido de un Pokémon.",
      version_group: {
        name: "lets-go-pikachu-lets-go-eevee",
        url: "https://pokeapi.co/api/v2/version-group/19/",
      },
    },
    {
      language: { name: "it", url: "https://pokeapi.co/api/v2/language/8/" },
      text: "Aumenta di poco i PP massimi di una mossa\na scelta tra quelle conosciute dal Pokémon.",
      version_group: {
        name: "lets-go-pikachu-lets-go-eevee",
        url: "https://pokeapi.co/api/v2/version-group/19/",
      },
    },
    {
      language: { name: "en", url: "https://pokeapi.co/api/v2/language/9/" },
      text: "A medicine that slightly raises the maximum PP\nof a single selected move that has been learned by\na Pokémon.",
      version_group: {
        name: "lets-go-pikachu-lets-go-eevee",
        url: "https://pokeapi.co/api/v2/version-group/19/",
      },
    },
    {
      language: { name: "ja", url: "https://pokeapi.co/api/v2/language/11/" },
      text: "ポケモンが　覚えている\n技のうち　１つの　ＰＰの\n最大値を　少し　あげる。",
      version_group: {
        name: "lets-go-pikachu-lets-go-eevee",
        url: "https://pokeapi.co/api/v2/version-group/19/",
      },
    },
    {
      language: { name: "zh-Hans", url: "https://pokeapi.co/api/v2/language/12/" },
      text: "能让宝可梦学会的\n其中１个招式的\nPP最大值少量提高。",
      version_group: {
        name: "lets-go-pikachu-lets-go-eevee",
        url: "https://pokeapi.co/api/v2/version-group/19/",
      },
    },
    {
      language: { name: "ja-Hrkt", url: "https://pokeapi.co/api/v2/language/1/" },
      text: "ポケモンが　おぼえている\nわざのうち　１つの　ＰＰの\nさいだいちを　すこし　あげる。",
      version_group: { name: "sword-shield", url: "https://pokeapi.co/api/v2/version-group/20/" },
    },
    {
      language: { name: "ko", url: "https://pokeapi.co/api/v2/language/3/" },
      text: "포켓몬이 기억하고 있는\n기술 중 1개의 PP\n최대치를 조금 올린다.",
      version_group: { name: "sword-shield", url: "https://pokeapi.co/api/v2/version-group/20/" },
    },
    {
      language: { name: "zh-Hant", url: "https://pokeapi.co/api/v2/language/4/" },
      text: "能讓寶可夢已學會的\n其中１個招式的\nＰＰ最大值少量提高。",
      version_group: { name: "sword-shield", url: "https://pokeapi.co/api/v2/version-group/20/" },
    },
    {
      language: { name: "fr", url: "https://pokeapi.co/api/v2/language/5/" },
      text: "Augmente un peu les PP max d’une capacité\nsélectionnée.",
      version_group: { name: "sword-shield", url: "https://pokeapi.co/api/v2/version-group/20/" },
    },
    {
      language: { name: "de", url: "https://pokeapi.co/api/v2/language/6/" },
      text: "Erhöht die maximale Anzahl der AP einer Attacke\neines Pokémon.",
      version_group: { name: "sword-shield", url: "https://pokeapi.co/api/v2/version-group/20/" },
    },
    {
      language: { name: "es", url: "https://pokeapi.co/api/v2/language/7/" },
      text: "Aumenta ligeramente los PP máximos del movimiento\nelegido de un Pokémon.",
      version_group: { name: "sword-shield", url: "https://pokeapi.co/api/v2/version-group/20/" },
    },
    {
      language: { name: "it", url: "https://pokeapi.co/api/v2/language/8/" },
      text: "Aumenta di poco i PP massimi di una mossa\na scelta tra quelle conosciute dal Pokémon.",
      version_group: { name: "sword-shield", url: "https://pokeapi.co/api/v2/version-group/20/" },
    },
    {
      language: { name: "en", url: "https://pokeapi.co/api/v2/language/9/" },
      text: "A medicine that slightly raises the max PP of a single\nselected move that has been learned by a Pokémon.",
      version_group: { name: "sword-shield", url: "https://pokeapi.co/api/v2/version-group/20/" },
    },
    {
      language: { name: "ja", url: "https://pokeapi.co/api/v2/language/11/" },
      text: "ポケモンが　覚えている\n技のうち　１つの　ＰＰの\n最大値を　少し　あげる。",
      version_group: { name: "sword-shield", url: "https://pokeapi.co/api/v2/version-group/20/" },
    },
    {
      language: { name: "zh-Hans", url: "https://pokeapi.co/api/v2/language/12/" },
      text: "能让宝可梦学会的\n其中１个招式的\nＰＰ最大值少量提高。",
      version_group: { name: "sword-shield", url: "https://pokeapi.co/api/v2/version-group/20/" },
    },
  ],
  fling_effect: null,
  fling_power: 30,
  game_indices: [
    {
      game_index: 69,
      generation: { name: "generation-iii", url: "https://pokeapi.co/api/v2/generation/3/" },
    },
    {
      game_index: 51,
      generation: { name: "generation-iv", url: "https://pokeapi.co/api/v2/generation/4/" },
    },
    {
      game_index: 51,
      generation: { name: "generation-v", url: "https://pokeapi.co/api/v2/generation/5/" },
    },
    {
      game_index: 51,
      generation: { name: "generation-vi", url: "https://pokeapi.co/api/v2/generation/6/" },
    },
    {
      game_index: 51,
      generation: { name: "generation-vii", url: "https://pokeapi.co/api/v2/generation/7/" },
    },
    {
      game_index: 51,
      generation: { name: "generation-viii", url: "https://pokeapi.co/api/v2/generation/8/" },
    },
  ],
  held_by_pokemon: [],
  id: 51,
  machines: [],
  name: "pp-up",
  names: [
    {
      language: { name: "ja-Hrkt", url: "https://pokeapi.co/api/v2/language/1/" },
      name: "ポイントアップ",
    },
    { language: { name: "ko", url: "https://pokeapi.co/api/v2/language/3/" }, name: "포인트업" },
    {
      language: { name: "zh-Hant", url: "https://pokeapi.co/api/v2/language/4/" },
      name: "ＰＰ提升劑",
    },
    { language: { name: "fr", url: "https://pokeapi.co/api/v2/language/5/" }, name: "PP Plus" },
    { language: { name: "de", url: "https://pokeapi.co/api/v2/language/6/" }, name: "AP-Plus" },
    { language: { name: "es", url: "https://pokeapi.co/api/v2/language/7/" }, name: "Más PP" },
    { language: { name: "it", url: "https://pokeapi.co/api/v2/language/8/" }, name: "PP-Su" },
    { language: { name: "en", url: "https://pokeapi.co/api/v2/language/9/" }, name: "PP Up" },
    {
      language: { name: "ja", url: "https://pokeapi.co/api/v2/language/11/" },
      name: "ポイントアップ",
    },
    {
      language: { name: "zh-Hans", url: "https://pokeapi.co/api/v2/language/12/" },
      name: "ＰＰ提升剂",
    },
  ],
  sprites: {
    default: "https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/items/pp-up.png",
  },
};

const defaultItemHandler = () => {
  return rest.get("https://pokeapi.co/api/v2/item/pp-up", (req, res, ctx) => {
    return res(ctx.json(mockedDefaultItemData));
  });
};

export const mockedDefaultItem = {
  data: mockedDefaultItemData,
  handler: defaultItemHandler,
};
