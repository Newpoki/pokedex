...
import { PokemonList } from "../../../pokemons/typings";
import { mockedDefaultPokemons } from "./default";

const mockedSecondPagePokemonsData: PokemonList = {
  count: 1126,
...
const secondPagePokemonsHandler = () => {
  return rest.get("https://pokeapi.co/api/v2/pokemon", (req, res, ctx) => {
    return res(ctx.json(mockedSecondPagePokemonsData));
  });
};

const defaultInfiniteQueryHookData = {
  pageParams: [undefined, mockedDefaultPokemons.infiniteQueryHookData.pages[0].next],
  pages: [mockedDefaultPokemons.data, mockedSecondPagePokemonsData],
};

export const mockedSecondPagePokemons = {
  data: mockedSecondPagePokemonsData,
  handler: secondPagePokemonsHandler,
  infiniteQueryHookData: defaultInfiniteQueryHookData,
};
